...
class HomeAssistantCapabilityContext:
    def __init__(self, catalog, allowed_read_entities=(), allowed_action_entities=(), allowed_services=(), aliases=None, maximum_items=500):
        if not isinstance(maximum_items, int) or not 1 <= maximum_items <= 500:
            raise ValueError("maximum_items must be between 1 and 500")
