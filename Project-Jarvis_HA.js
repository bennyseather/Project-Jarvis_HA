const JARVIS_UI_VERSION = "0.47.1";
...
class JarvisThermalGraphCard extends JarvisBaseCard {
  ...
  render() {
    ...
    this.shell(`<div class="thermal"><header><div><div class="eyebrow">Historical thermal envelope</div><div class="name">${escapeHtml(this._config.name || "Jarvis Node thermals")}</div></div><ha-icon icon="mdi:chart-multiple"></ha-icon></header><div class="thermal-plot"><span>HISTORY LOADING</span></div><div class="legend">${legend}</div></div><style>.thermal{--series-0:var(--j-accent);--series-1:color-mix(in srgb,var(--j-accent) 68%,#eafaff);--series-2:color-mix(in srgb,var(--j-accent) 72%,#315cff);--series-3:color-mix(in srgb,var(--j-accent) 60%,#55e6a5);--series-4:color-mix(in srgb,var(--j-accent) 65%,#ffc247);--series-5:color-mix(in srgb,var(--j-accent) 65%,#ff6572);min-height:220px;padding:18px;display:grid;gap:12px}.thermal header{display:flex;justify-content:space-between;align-items:center}.thermal header ha-icon{color:var(--j-accent)}.thermal-plot{height:120px;border-left:1px solid var(--j-line);border-bottom:1px solid var(--j-line);display:grid;place-items:center}.thermal-plot>span{font:600 8px monospace;color:var(--secondary-text-color)}.thermal-plot svg{width:100%;height:100%;overflow:visible}.thermal-plot polyline{fill:none;stroke-width:2;vector-effect:non-scaling-stroke;filter:drop-shadow(0 0 3px currentColor)}.thermal-plot .series-0{stroke:var(--series-0)}.thermal-plot .series-1{stroke:var(--series-1)}.thermal-plot .series-2{stroke:var(--series-2)}.thermal-plot .series-3{stroke:var(--series-3)}.thermal-plot .series-4{stroke:var(--series-4)}.thermal-plot .series-5{stroke:var(--series-5)}.legend{display:flex;flex-wrap:wrap;gap:8px 14px}.legend span{display:grid;grid-template-columns:8px auto auto;gap:5px;align-items:center;font:600 9px monospace}.legend i{width:7px;height:7px;background:var(--j-accent)}.legend .series-1 i{background:var(--series-1)}.legend .series-2 i{background:var(--series-2)}.legend .series-3 i{background:var(--series-3)}.legend .series-4 i{background:var(--series-4)}.legend .series-5 i{background:var(--series-5)}.legend b{color:var(--j-accent)}</style>`, { interactive:false });
    if (this._visible) this.loadHistory();
  }
  async loadHistory() {
    ...
    const values=entry.series.flat();if(values.length<2){host.innerHTML="<span>NO THERMAL HISTORY</span>";return;}const min=Math.floor(Math.min(...values)-2),max=Math.ceil(Math.max(...values)+2),span=Math.max(1,max-min);const lines=entry.series.map((series,index)=>series.length<2?"":`<polyline class="series-${index}" points="${series.map((v,i)=>`${(i/(series.length-1)*100).toFixed(1)},${(96-(v-min)/span*88).toFixed(1)}`).join(" ")}"></polyline>`).join("");host.innerHTML=`<svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="${hours} hour thermal history">${lines}</svg>`;
  }
}
