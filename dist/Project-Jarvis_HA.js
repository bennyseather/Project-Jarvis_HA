const JARVIS_UI_VERSION = "0.14.0";
...
class JarvisButtonCard extends JarvisBaseCard {
  ...
  render() {
    ...
    this.shell(`<div class="button-layout"><div class="icon-shell"><ha-icon icon="${escapeHtml(this._config.icon || "jarvis:button")}"></ha-icon></div><div><div class="eyebrow">Command node</div><div class="name">${escapeHtml(label)}</div><div class="state">${escapeHtml(this._config.description || "Ready")}</div></div><div class="chev">â€º</div></div>
...
class JarvisClimateCard extends JarvisBaseCard {
  ...
  render() {
    ...
    this.shell(`<div class="climate-layout">${this.entityHeader("Climate regulation")}<div class="temp">${escapeHtml(formatValue(attrs.current_temperature))}Â°<small>CURRENT</small></div><div class="target"><span>TARGET ${escapeHtml(formatValue(temp))}Â°</span><input aria-label="Target temperature" type="range" min="${attrs.min_temp ?? 5}" max="${attrs.max_temp ?? 35}" step="${attrs.target_temp_step ?? .5}" value="${temp}"></div></div>
