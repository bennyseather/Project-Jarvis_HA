...
class JarvisUIDesignSystemTests(unittest.TestCase):
    ...
    def test_camera_starts_live_and_voice_uses_shared_hud_framing(self):
        ...
        self.assertIn("host.replaceChildren(card);", self.script)
        self.assertIn("card.requestUpdate?.();", self.script)
        self.assertIn("camera_proxy/${this._config.entity}", self.script)
        self.assertIn("setInterval(refresh, 5000)", self.script)
        self.assertIn('class="jarvis-hud-frame ', self.script)
        self.assertIn("border-radius:2px!important", self.script)
    ...
    def test_cards_have_gutters_and_tall_cards_reserve_enough_grid_rows(self):
        self.assertIn("padding:6px", self.script)
        self.assertIn("height:100%", self.script)
        self.assertIn("ha-card>div{height:100%;min-height:0!important", self.script)
        self.assertIn("min_rows: rows", self.script)
        self.assertIn("static gridRows = 4", self.script)
        self.assertIn("return { rows: 5, columns: 6, min_rows: 5", self.script)
    ...
    def test_light_card_uses_bulb_icon_as_toggle(self):
        ...
        self.assertIn(
            "grid-template-columns:54px minmax(0,1fr)", self.script
        )
        self.assertIn("@media(max-width:900px)", self.script)
        self.assertIn(
            "grid-template-columns:58px minmax(0,1fr)", self.script
        )
        self.assertIn(".light-toggle{width:56px;height:56px", self.script)
